...
- **`RESOURCE_URI`의 `UI_VERSION`은 UI가 변경될 때마다 반드시 올린다.** 호스트가 `ui://` 리소스를 URI 기준으로 캐싱하므로, 버전을 올리지 않으면 배포는 성공했는데 옛 HTML이 계속 사용된다. 클라이언트 번들이 바뀌는 변경(에셋 맵 확장 등)도 UI 변경으로 간주한다. 코드와 같은 커밋에 넣는다.
...
**M4 (UX 다듬기) 완료.** `main` @ `78d0fe5`, 프로덕션 배포 READY.
...
- M4 — 변형 에셋이 호출마다 다르게 나오는 것 확인 ✅

미완성된 것:

- 안정화 검증(cold start, 연속 호출, connector 재연결, 프로토콜 버전 호환성)이 안 됐다.
- 호출 빈도가 적절한지는 실사용 누적 후에야 판단할 수 있다. 너무 잦거나 드물면 preference 문구를 조정한다.
- README가 초기 상태다. 구조 설명과 Claude.ai 등록법이 없다.
...
1. 안정화 검증: cold start, 연속 호출, connector 재연결, 잘못된 emotion 입력, 모바일 렌더링.
2. README 정리: 구조 설명, Claude.ai 커넥터 등록법, 감정 추가 방법.
3. `middleware` → `proxy` 파일명 전환 (선택).
4. Railway 리포 아카이브.
